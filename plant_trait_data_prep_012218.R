...
plot_data_no_equals$Taper <- plot_data_no_equals$Ht / plot_data_no_equals$Max_D30
...
charlight[charlight$Number == 662, c("Char.height", "Light.code")] <- c(NA, NA) #blank on datasheet
charlight[charlight$Number %in% c(8056, 8057), "Light.code"] <- NA #blank on datasheet
charlight[charlight$Number %in% c(1330, 1337), "Light.code"] <- NA #marked as 0 on datasheet
...
clean_species <- data.frame(Code = unique(clean_data$Code),
                            ...
                            Taper = numeric(length(unique(clean_data$Code))),
...
for(i in 1:length(unique(clean_data$Code))){
  ...
  clean_species$Taper[i] <- mean(data_select$Taper, na.rm = TRUE)
